...
public class EmployeeService : IEmployeeService
{
    ...
    public async Task<List<EmployeeDto>> GetNextEmployeesAsync(Guid id, CancellationToken cancellationToken)
    {
        ...
        var restOfEmployees = await _context.Employees
            .Include(x => x.UserTags)
            .ThenInclude(x => x.Tag)
            //.Where(x=> x.Matches.Any(y=>y.JobId!=employer.Id)) jak nikt nie ma matchy to wypierdala go�cia
        ...
        return employees.Select(Mappers.MapToEmployeeDto).ToList(); // tu powinno by� dto !!
    }
