...
from app.models import Matches, Tags, Usertags, Aspnetusers, Employertags
...
class MatchViewSet(viewsets.ModelViewSet):
    ...
    def get_employees_list(self, request, pk=None):
        tags = Tags.objects.all()
        matches = Matches.objects.filter(employerid=pk).order_by('createdat')[:100].all()
        ...
        for t in tag_map:
            l[t] = []
            for m in matches:
                if t not in m.userid.usertags.tagid.name.split('\n'):
                    l[t].append(0)
                else:
                    l[t].append(1)

        l["accepted"] = [m.didemployeracceptcandidate for m in matches]
        df = pd.DataFrame(l)
        x = df.drop("accepted", axis=1)
        x = x.values
        y = df["accepted"]
        ...
        user_tags = Employertags.objects.filter(employerid=pk).all().select_related('tagid')
        tag_ids = user_tags.values_list('tagid', flat=True)
        users = Aspnetusers.objects.filter(usertags__tagid__id__in=tag_ids)[:10]

        if users.count() == 0:
            users = Aspnetusers.objects.all()[:10]
        ...
        for e in users:
            ...
            for t in tag_map:
                try:
                    if t not in users.employertags.tagid.name.split('\n'):
                        x.append(0)
                    else:
                        x.append(1)
                except:
